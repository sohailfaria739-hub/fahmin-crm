...
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || 'AQ.Ab8RN6ICZMnvyyWvJmkk-_bnDlZFZqryTuZS' });

async function buildContext(userId) {
  try {
    const [contactsSnap, dealsSnap, tasksSnap, callsSnap] = await Promise.all([
      db.collection('contacts').where('user_id', '==', userId).get(),
      db.collection('deals').where('user_id', '==', userId).get(),
      db.collection('tasks').where('user_id', '==', userId).where('done', '==', 0).get(),
      db.collection('calls').where('user_id', '==', userId).where('status', '==', 'pending').get()
    ]);
    ...
    return 'Error loading CRM data snapshot.';
  }
}
